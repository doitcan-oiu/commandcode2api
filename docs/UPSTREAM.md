...
| 上次检查日期 | 2026-09-20（Asia/Shanghai） |
...
- 协议代理使用 Go 标准库 HTTP；多阶段 Docker 构建产出静态二进制，非 root 运行并内置探活。后续聚合层新增的 SQLite 和前端依赖见下文。
...
此次聚合功能属于本 fork 的本地扩展，不代表上游新增提交，也不改变上述已移植基线：

- 默认启用账号池和 React、shadcn/ui、Tailwind CSS 管理界面，服务端保管多个上游 `user_...` 密钥并向客户端发放独立 `ccg_...` 令牌。面向单管理员与团队 API 客户端；不包含多管理员角色或商业充值计费。
- `internal/gateway` 负责管理员初始化/登录、账号和令牌管理、调度与冷却、下游模型/速率/并发/累计配额、请求元数据和 SQLite 存储。`internal/usage` 查询并归一身份、余额、订阅和累计用量。`internal/webui` 托管 `web/dist`，前端源码在 `web`。
- 调度支持按窗口剩余额度调整权重、加权轮询和按并发负载选取；较高优先级的可用账号优先。上游鉴权失效、额度或速率限制、暂时故障分开处理；流式响应开始后不能自动重播。
- 聚合模式的上游会话按客户端令牌和账户隔离，并为派生 ID 设置 UUID v4 的版本位与 RFC variant 位。仅将哈希排列成 `8-4-4-4-12` 会被上游 `threadId` 校验以 400 拒绝；此问题已于 2026-09-20 复现并修复，三种协议的回归测试都包含严格 UUID 校验。
- 用量未知值保留为未知，部分上游端点失败保留其余数据。`limited` 字段本身不等于额度耗尽；社区套餐月额度仅作为带 estimated 标记的展示值。用量归一参考用户提供的 `commandcode-usage` 项目，版权见根目录 `THIRD_PARTY_NOTICES.md`；参考目录不作为运行时依赖。
- 下游请求配额在接受请求时预留，随后失败也计数；Token 配额按完成请求的上游 usage 结算，在途并发可能造成超额，并非严格 Token 预扣费。请求日志保存元数据，不保存请求/响应正文或完整凭据。
- 上游密钥在 SQLite 内使用 AES-GCM 加密，`master.key` 与数据库放在 `CC_DATA_DIR`；备份和恢复必须同时保留。下游令牌保存摘要，管理员密码保存密码哈希。按用户 2026-09-20 的要求，首次安装直接在页面设置用户名和至少 8 位密码，无需 setup token；创建后关闭初始化入口。仍可通过 `CC_ADMIN_PASSWORD` 初始化 `admin` 账号。
- `CC_GATEWAY_ENABLED=false` 可回到原有每请求携带上游 Key 的兼容模式。聚合模式下 `/` 是管理界面，兼容模式下 `/` 仍是健康检查别名；两种模式始终保留 `/health`。
- 后端新增纯 Go SQLite 依赖，因此“后端只使用标准库”仅描述最初迁移版本。当前部署依然不要求 C 编译或 Node 运行时。Docker 使用独立 Node 前端构建阶段和 Go 构建阶段，通过 `COPY` 本地源码构建，最终镜像携带静态资源并挂载持久化数据卷。

移植上游改动时，特别检查认证、模型过滤、会话/指纹、流式响应生命周期、错误重试和 usage 字段与本地聚合层的交互。不得用上游的单 Key 入口覆盖聚合入口，也不得误删 `CC_GATEWAY_ENABLED=false` 兼容路径。构建和初始化步骤见根目录 README。
...
本地聚合扩展的验证应另外覆盖多账号调度、冷却恢复、下游令牌认证/限额、管理员初始化与会话、持久化和凭据保护，以及前端构建与交互。最初迁移的通过记录不能当作这些新增功能的验证结论；以当前修改实际执行的检查为准。
